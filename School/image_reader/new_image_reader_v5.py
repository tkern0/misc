...
class MainWindow():
    def __init__(self, root):
        ...
        self.write_button = tk.Button(frame, text="Write Current Image Data to CSV File", command=self._write_to_csv)
    ...
    def _get_img_path(self):
        p = filedialog.askopenfilename(filetypes=(("jpeg files", "*.jpg"), ("tiff files","*.tiff")), defaultextension=".jpg")
    ...
    def _get_csv_path(self):
        p = filedialog.askopenfilename(filetypes=(("csv files", "*.csv"),), defaultextension=".csv")
    ...
    def _write_to_csv(self):
        # First, update the image data, just in case
        self._update_data()
        ...
        try:
            ...
            size = 0
        # If we have an empty/non-existant file we want to create it, leaving a
        #  header at the top and then including our data
        if size == 0:
            header = sorted(list(self.exif.keys()))
            with csv.DictWriter(open(self.csv, "w"), fieldname=header) as csv_file:
                csv_file.writeheader()
                csv_file.writerow(self.exif)
            return
        # Otherwise we want to append to the file
        # First we get the header
        with csv.reader(open(self.csv, "r")) as csv_file:
            header = next(csv_file)
        # Now we check if the header covers all entries we have in data
        new_entries = 0
        for i in self.exif:
            if not i in header:
                # Appending so that we don't change the order of existing stuff
                header.append(i)
                new_entries += 1
        # If we have a new entry we need to re-create the whole file, so we
        #  need to get all current data first
        if new_entries > 0:
            with csv.DictReader(open(self.csv, "r")) as csv_file:
                csv_data = []
                for row in csv_file:
            ...
            with csv.DictWriter(open(self.csv, "w"), fieldname=header) as csv_file:
                 ...
                 csv_file.writerow(self.exif)
        # If we don't have any new entries we can just append our data
        else:
